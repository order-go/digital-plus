...
const PaymentModal: React.FC<PaymentModalProps>=({total,onConfirm,onClose}: PaymentModalProps) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 ">
            <div className=" w-screen border-2 border-primary text-textSecundary bg-tertiary p-4 rounded-lg shadow-lg">
                <div className='flex flex-row justify-between items-center'>
                    <h2 className="text-xl font-bold mb-4">Información de Pago</h2>
                    <IoMdCopy className='mb-4 w-6 h-6 cursor-pointer active:text-textPrimary' onClick={handleCopy} />
                </div>
                <div className='flex flex-col justify-center items-center'>

                    <strong className='underline'>ADVERTENCIA </strong>
                    <p className=' text-sm justify-center  mb-2'>
                ...
                <div className="flex justify-center space-x-2">
                    <Button onClick={(): void => onConfirm(referenceNumber)} label="Confirmar" color="bg-textPrimary active:bg-secundary text-white" />
